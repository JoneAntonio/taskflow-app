...
export async function POST(request: Request) {
  ...
  try {
    await sendEmail({
      to: profile.email,
      subject: `${senderProfile?.full_name ?? "Alguém"} mencionou-te no JAFLOW`,
      html: `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;">
        <h2 style="color:#14151b;">Foste mencionado</h2>
        <p style="color:#3a3a3a;font-size:14px;">${senderProfile?.full_name ?? "Alguém"} escreveu:</p>
        <p style="color:#3a3a3a;font-size:14px;background:#f7f7f4;border-radius:8px;padding:12px;">${snippet ?? ""}</p>
